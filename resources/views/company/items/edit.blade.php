        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-2xl font-semibold text-gray-800">Edit Item: {{ $item->name }}</h2>
                ...
                <form action="{{ route('company.items.update', $item) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                                ...
                                <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                    ...
                                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                        <textarea id="description" name="description" rows="3" 
                                            class="shadow-sm focus:ring-blue-500 focus:border-blue-500 block w-full sm:text-sm border border-gray-300 rounded-md">{{ old('description', $item->description) }}</textarea>
                                    </dd>
                                </div>
                                ...
                                <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                    ...
                                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-1">
                                        <select id="unit" name="unit" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md">
                                            ...
                                            <option value="g" {{ old('unit', $item->unit) == 'g' ? 'selected' : '' }}>Grams</option>
                                            <option value="l" {{ old('unit', $item->unit) == 'l' ? 'selected' : '' }}>Liters</option>
                                            <option value="ml" {{ old('unit', $item->unit) == 'ml' ? 'selected' : '' }}>Milliliters</option>
                                            <option value="m" {{ old('unit', $item->unit) == 'm' ? 'selected' : '' }}>Meters</option>
                                            <option value="cm" {{ old('unit', $item->unit) == 'cm' ? 'selected' : '' }}>Centimeters</option>
                                    ...
                                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-1">
                                        <div class="mt-1 relative rounded-md shadow-sm">
                                            ...
                                            <input type="number" name="purchase_price" id="purchase_price" 
                                                value="{{ old('purchase_price', number_format($item->purchase_price, 2)) }}" step="0.01" min="0" required
                                    ...
                                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-1">
                                        <div class="mt-1 relative rounded-md shadow-sm">
                                            ...
                                            <input type="number" name="selling_price" id="selling_price" 
                                                value="{{ old('selling_price', number_format($item->selling_price, 2)) }}" step="0.01" min="0" required
                                                class="focus:ring-blue-500 focus:border-blue-500 block w-full pl-7 pr-12 sm:text-sm border-gray-300 rounded-md" 
                                                placeholder="0.00">
                                        </div>
                                    </dd>
                                ...
                                <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                    ...
                                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                        <textarea id="notes" name="notes" rows="3" 
                                            class="shadow-sm focus:ring-blue-500 focus:border-blue-500 block w-full sm:text-sm border border-gray-300 rounded-md">{{ old('notes', $item->notes) }}</textarea>
                                    </dd>
                                </div>
                            </dl>
                        </div>
