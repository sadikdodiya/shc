    ...
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-lg shadow-md p-6">
            <div class="flex justify-between items-center mb-6">
                ...
                <a href="{{ route('company.complaints.index') }}" class="text-gray-500 hover:text-gray-700">
            ...
            <form action="{{ route('company.complaints.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="product_id" id="product_id" required 
                            ...
                            @foreach($products as $product)
                                <option value="{{ $product->id }}" {{ old('product_id') == $product->id ? 'selected' : '' }} data-brand="{{ $product->brand_id }}">{{ $product->name }}</option>
                            @endforeach
                        </select>
                        @error('product_id')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <select name="fault_type_id" id="fault_type_id" required
                            ...
                            @foreach($faultTypes as $faultType)
                                <option value="{{ $faultType->id }}" {{ old('fault_type_id') == $faultType->id ? 'selected' : '' }}>{{ $faultType->name }}</option>
                            @endforeach
                        </select>
                        @error('fault_type_id')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <select name="call_type" id="call_type" required
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                            <option value="warranty" {{ old('call_type') == 'warranty' ? 'selected' : '' }}>Warranty</option>
                            <option value="paid" {{ old('call_type') == 'paid' ? 'selected' : '' }}>Paid Service</option>
                            <option value="amc" {{ old('call_type') == 'amc' ? 'selected' : '' }}>AMC</option>
                        </select>
                        @error('call_type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="text" name="city" id="city" value="{{ old('city', auth()->user()->company->city) }}" required
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                        @error('city')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="text" name="state" id="state" value="{{ old('state', auth()->user()->company->state) }}" required
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                        @error('state')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="col-span-2 md:col-span-1">
                        ...
                        <select name="assigned_staff_id" id="assigned_staff_id"
                            ...
                            @foreach($staff as $member)
                                <option value="{{ $member->id }}" {{ old('assigned_staff_id') == $member->id ? 'selected' : '' }}>{{ $member->name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="mt-8 flex justify-end space-x-3">
                    <a href="{{ route('company.complaints.index') }}" class="bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                        Cancel
                    </a>
